...
string    texture = "TRU POOFER BALLOONS";          // Texture used for particles. Texture must be in prim's inventory.
...
float    rate = 0.2;   // Delay between bursts of new particles, 0.0 to 60, 0.1(*)
...
float      radius = 1.0;      // 0.0(default) to 64?  Distance from Emitter where new particles are created.
