...
// :CODE:
// This is Rev 9.3, based on rev 4.6  - skipped way up because of custom tweaks.
...
StateSpawn() {
   ...
   // llSay(0,llDumpList2String(name,",")); 
    ...
    llMessageLinked(LINK_SET,-10,(string) llGetLinkNumber() + "|" + (string) llGetPos() ,aKey);    // broadcast the key on num = -1
...
default
{
    ...
    link_message(integer sender, integer num, string str, key id){
        ...
        if (num == 0) {
            ParseMsg(str);
            llMessageLinked(LINK_SET,-10,(string) llGetLinkNumber() + "|" + (string) llGetPos() ,NPCKey());    // broadcast the key on num = -1
        }
        if (num == 99)
            llResetScript();
    }
