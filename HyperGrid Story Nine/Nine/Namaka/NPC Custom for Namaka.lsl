...
// This is Rev 9.3 based on 4.5  09/26/2015
...
//////////////////////////////////////////////////////////
integer keyNum = -1;    // (dylan) special number for link message to broadcast the NPC key
...
DoTeleport(string params) {
    vector Dest = (vector) params;
    
    DEBUG("teleport to " + (string) Dest);
    
    if (Dest != ZERO_VECTOR) {
        if (relAbs == "Relative"){
            Dest += llGetPos();
        }
        DEBUG("Off to " + (string) Dest);
