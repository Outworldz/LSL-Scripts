...
float     RANGE = 10;        // 1 to N meters  - anyone this close to the controller will start NPCS if Sensor button is clicked
...
StateSpawn() {
    ...
    DEBUG("Rezzing the  NPC:" + (string) vRezPos);
...
ProcessCmd(string cmd) {
    ...
    if(npcAction == "@spawn" && avatarPresent) {
        DEBUG("@spawn");
