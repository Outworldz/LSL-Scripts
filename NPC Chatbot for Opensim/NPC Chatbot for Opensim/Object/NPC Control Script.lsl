...
//////////////////////////////////////////////////////////
integer   allowUsers = TRUE;   // If true, any user can get a Start NPC and Stop NPC menu.  Only groups and owners canb get all commands if TRUE, or FALSE
...
integer   WAIT = 10;           // wait for this number of seconds for the NPC to reach a destination (for safety). If it fails to reach a target, it will move on after this time.
float     RANGE = 35.0;        // 1 to 96.0 meters  - anyone this close to the controller will start NPCS if  Sensor button is clicked
...
// "se" is "on" if sensors are enabled
...
Kill(string someNPCName)
{
    ...
    if (count)
        ...
        llOwnerSay("Could not locate " + someNPCName);
}
...
{
    menu();
    list buttons = ["Appearance","Recording","Save","Help","-","Erase RAM", priPub,relAbs,"-","Stop NPC","Sensor","Start NPC"];
...
state spawn
{
    state_entry() {
       ...
       // notecard is stored as offsets from this box with relative addressing.  Convert to absolute
        ...
        llMessageLinked(LINK_SET,2,"Key",NPCKey());
