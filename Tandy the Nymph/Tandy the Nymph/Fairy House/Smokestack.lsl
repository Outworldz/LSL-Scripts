...
{                
    ...
    ,PSYS_SRC_ACCEL,             <0.0,0,.1>    //Acceleration of particles each second
    ...
    ,PSYS_PART_START_SCALE,      <.3,.3,.1>  //Starting particle size
    ,PSYS_PART_END_SCALE,        <.6,.6,.2>  //Ending particle size, if INTERP_SCALE_MASK is on
...
default
{
    state_entry()
    {
        smoke = llGetInventoryName(INVENTORY_TEXTURE,0);
        llSetTimerEvent(2.0);
    }
