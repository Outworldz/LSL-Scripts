...
float  startAlpha = 0.08;         // 0.0 to 1.0(*), lower = more transparent
float    endAlpha = 0.04;         // 
vector  startSize = <2,2,0>; // <0.04,0.04,0>(min) to <10,10,0>(max>, <1,1,0>(*)
vector    endSize = <2,2,0>; // (Z part of vector is discarded)  
string    texture = "smokey";          // Texture used for particles. Texture must be in prim's inventory.
...
float    rate = 0.50;   // Delay between bursts of new particles, 0.0 to 60, 0.1(*)
float     age = 030.0;   // How long each particle lives, 0.1 to 60, 10.0(*)
...
float       maxSpeed = 0.5;     // 0.01 to ? Max speed each particle is spit out at, 1.0(*)
